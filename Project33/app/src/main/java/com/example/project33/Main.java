...
public class Main extends MyBaseActivity
    ...
    public void buttonAdd_Click(View view){
        ...
        intent.putExtra(Intent.EXTRA_TIME, "");
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        ...
        intent.putExtra(EXTRA_TIME, myApp.getNote(position)[2]);
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if(resultCode == RESULT_OK){
            ...
            switch (requestCode){
                case CREATE_ACTION:
                    myApp.addInList(headtext, text, priority, time);
                    break;
                case EDIT_ACTION:
                    int position = bundle.getInt(EXTRA_ID);
                    myApp.setInList(position, headtext, text, priority, time);
                    break;
            }
            myApp.loadNotes();
